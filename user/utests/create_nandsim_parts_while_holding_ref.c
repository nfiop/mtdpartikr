...
#define TEST_NAME "create_nandsim_parts_while_holding_ref"
...
int main(int argc, char *argv[])
{
	...
	/* Get a ref on the proxy MTD */
	{
		ret = snprintf(mtd_path, sizeof(mtd_path), "/dev/mtd%u",
		    mtd_info.proxy_mtd_index);
		ret = ret < 0 ? -EINVAL : 0;
	...
	for (int i = 0; i < 20; i++) {
		...
		SUBTEST_FAIL_IF_ERRNO_SET(
		    TEST_NAME, "adding-partition-via-context", ret);
	}
