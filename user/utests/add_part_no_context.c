...
int main(int argc, char *argv[])
{
	...
	TEST_FAIL_CONDITIONAL_IF_ERRNO_SET(TEST_NAME, "sysinfo", errno_ret);
	...
	TEST_FAIL_CONDITIONAL_IF_ERRNO_SET(TEST_NAME, "snprintf", errno_ret);

	INVOKE_IOCTL_WITH_RET_VARIABLE_AS_ERRNO(
	    fd, MTDPARTCTL_IOC_ADD_MTD_PARTITION, &part_info);
